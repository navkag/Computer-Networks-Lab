...
int main(int argc, char *argv[]){
	...
	while(true){
		...
		if (message == "-1"){
            sendto(clientSocket, message.c_str(), message.length(), 0, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
            cout << "Bye!\n";
            return 1;
        }
		...
		int bytesReceived = recvfrom(clientSocket, ackBuffer, sizeof(ackBuffer), 0, (struct sockaddr *)&serv_addr, &serverAddrLen);
		if (bytesReceived > 0) {
